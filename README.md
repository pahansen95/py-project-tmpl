...
the `origin` remote, `-b` to select a template branch, and `--push` to
immediately push the initial commit. The script deletes itself after
committing so it won't pollute your new project.

```bash
curl -L https://raw.githubusercontent.com/pahansen95/py-project-tmpl/trunk/quickstart.sh \
  | bash -- -C ./project/path -u git@github.com:username/project.git -b trunk --push
```
